...
void main() {
    ...
    vec2 texCoord = vTexCoord * 2.0 - 1.0; // -1.0 ~ 1.0
    ...
    vec4 bottom  = texture2D(uTexture, vTexCoord - vec2(0.0, 1.0) * fragment);
    vec4 left    = texture2D(uTexture, vTexCoord - vec2(1.0, 0.0) * fragment);
    vec4 right   = texture2D(uTexture, vTexCoord + vec2(1.0, 0.0) * fragment);
    ...
    float velocity = current.g + accel;
